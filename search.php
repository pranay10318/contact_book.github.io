...
table, th, td {
  ...
  <body style="background-color: aquamarine;">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      ...
      <br>
    <div>      
        <strong ><h2 style="color: rgb(79, 163, 127);">You can search contacts by name/number/email<h1></strong>
    </div>
    <br>
    
    <form action="/dbms_project/search.php" method="POST" class="form-inline my-2 my-lg-0" >

    <input class="form-control mr-sm-2" name="entry" type="search" placeholder="Search" aria-label="Search">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if($entry==''){
    ...
    <strong>Search field cannot be empty..!</strong>
  </div>';
  $display=false;
  }
  ?>
...
<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  $sql="select *from  $username where firstname like '%$entry%' or lastname like '%$entry%' or number like '%$entry%'";
...
mysqli_close($conn);
// $sql = "SELECT *FROM contacts where firstname like '%$entry%'";
// $result = mysqli_query($conn, $sql);

// if (mysqli_num_rows($result) > 0) {
//   // output data of each row
//   while($row = mysqli_fetch_assoc($result)) {
//     echo "name: " . $row["firstname"]. " " . $row["lastname"]. " number " . $row["number"].$row["email"] . "<br>";
//   }
// } else {
//   echo "0 results";
// }
